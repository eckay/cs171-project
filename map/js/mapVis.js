class MapVis {
    ...
    tallyStatusByState()
    {
        ...
        vis.bookData.forEach(row => { // referenced gpt
            ...
            if (banStatus.toLowerCase() ===  vis.selectedCategory) {
    ...
    wrangleData() {
        ...
        vis.tallyStatusByState = vis.tallyStatusByState()
        vis.topBooksByState = vis.topBooksByState()
    ...
    updateVis() {
        let vis = this;
        ////console.log("testing indiv states", vis.stateInfo["Louisiana"])
        d3.selectAll(".state").each(function(d) { // d represents vis.world data bound to each element
           try
           {
               // console.log("tally", vis.topBooksByState[d.properties.name] ? vis.tallyStatusByState[d.properties.name] : 0)
               let tally =  vis.tallyStatusByState[d.properties.name]  ? vis.tallyStatusByState[d.properties.name] : 0
               ...
               vis.svg.append("text")
                   .text(tally === 0 ? "" : tally)
                   .attr("transform", d => `translate(${projection(d3.geoCentroid(d))[0]}, ${projection(d3.geoCentroid(d))[1]})`)
                   .attr("x", d => projection(d3.geoCentroid(d))[0]) // X-coordinate of state centroid
                   .attr("y", d => projection(d3.geoCentroid(d))[1]) // Y-coordinate of state centroid
               ...
               //console.log("region/state not in stateInfo: ", d.properties.name)
           }
        });
        ...
        d3.selectAll(".state").on('mouseover', function(event, d) {
            ...
            let {tooltipX, tooltipY} = vis.tooltip.node().getBoundingClientRect();
